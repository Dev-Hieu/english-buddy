...
const publicUser = (r: any): User => ({ id: r.id, email: r.email, name: r.name, role: r.role, createdAt: r.createdAt });
...
export function registerUser(email: string, password: string, name: string): { token: string; user: User } | { error: string } {
  ...
  const id = randomUUID();
  db.prepare("INSERT INTO users (id, email, passwordHash, name, role, createdAt) VALUES (?, ?, ?, ?, 'parent', ?)")
