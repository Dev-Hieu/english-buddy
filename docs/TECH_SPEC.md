...
> **CẬP NHẬT v0.3 (D-010):** Backend đổi từ Firebase sang **server tự host (Node + Express + SQLite)** trong `server/` — để chủ động + đồng bộ đa thiết bị. Các mục nhắc Firebase/Firestore/Functions bên dưới hiểu là **API server tự host**: dữ liệu lưu SQLite; proxy ảnh/dịch là route `/api/image`, `/api/translate`; auth là 1 mật khẩu phụ huynh. **CONTRACTS chữ ký service giữ nguyên** nên UI không đổi.
