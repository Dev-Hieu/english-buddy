...
export function DashboardPage({ students, onBackHome }: DashboardPageProps) {
  ...
  if (!student) return <main className="mx-auto max-w-xl px-4"><SessionHeader title="Tiến độ" onClose={onBackHome} /><p className="text-center text-sm text-muted-foreground py-8">Chưa có dữ liệu</p></main>;

  if (loading) return <main className="mx-auto max-w-xl px-4"><SessionHeader title="Tiến độ" onClose={onBackHome} /><div className="flex justify-center py-12"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div></main>;
  ...
  return (
    <main className="mx-auto w-full max-w-xl px-4">
      <SessionHeader title="Tiến độ học tập" onClose={onBackHome} />
